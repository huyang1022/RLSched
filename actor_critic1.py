    ...
    def __init__(self, sess, pa):
        ...
        with tf.variable_scope("Actor1"):
            ...
            with tf.variable_scope("Net"):
                ...
                l1 = tf.layers.dense(self.state, 128, tf.nn.relu6, name = "hidden_layer1")
            ...
            with tf.variable_scope("Loss"):
                ...
                log_prob = tf.reduce_sum(tf.multiply(self.act_prob, tf.squeeze(tf.one_hot(self.act, self.a_dim))), axis=1, keepdims=True)
                sum_prob = tf.multiply(tf.log(log_prob),  self.td_error)
                entropy = - tf.reduce_sum(tf.multiply(self.act_prob, tf.log(self.act_prob + self.pa.eps)), axis=1, keepdims=True)
                self.entropy =  tf.reduce_sum(entropy)
                self.loss = tf.reduce_mean( - (sum_prob + self.pa.entropy_rate * entropy))
